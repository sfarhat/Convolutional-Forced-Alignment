...
What does this mean wrt GRAD-CAM? Well, that method uses a weighted combination of activation maps, and if each timestep is looking at the entirety of the activation maps, then the interpretation is a bit weird. As I saw in an example, the CAM for a single class (phoneme) at a specific timestep is actually taking information from across a bunch of timesteps in the input spectrogram. 

Below is an example for a phoneme (class) that is present from timesteps 10 to 17, yet GRAD-CAM is saying that it's paying attention the most to timestep 80!

![Alt text](./cam.png)

It seems GRAD-CAM is not the move here with its reliance on layer activations. I guess this opens up the question of how should we add interpretability to a Convolutional model that works with sequential input? It is worth mentioning that while the last layer activations span the entire image, the last layer gradients with respect to a phoneme are exclusive to the timestep in which it appears. Perhaps I should just use a simplier saliency map or something that just uses the gradient. 